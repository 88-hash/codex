...
public interface UserMapper {
    ...
    @Insert("INSERT INTO user(phone, name, avatar, signature) VALUES(#{phone}, #{name}, #{avatar}, #{signature})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(User user);

    @Insert("INSERT INTO user(phone, name, avatar) VALUES(#{phone}, #{name}, #{avatar})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertLegacy(User user);

    @Update("UPDATE user SET name = #{name}, avatar = #{avatar}, signature = #{signature}, updated_at = NOW() WHERE id = #{id}")
    int update(User user);

    @Update("UPDATE user SET name = #{name}, avatar = #{avatar}, updated_at = NOW() WHERE id = #{id}")
    int updateLegacy(User user);
